...
# note that this function includes the studied item score in the total score for simplicity, but normally the item score would be excluded from the total score
item_total_r <- responses %>% 
  ...
  group_by(item) %>% 
  summarise(correlation = cor(score, total, use = "pairwise.complete.obs")) %>% 
